# Chunking gorsel test scripti.
# Kullanim: .venv/Scripts/python scripts/chunk_inspect.py
...
# ─── AYARLAR (isteğe bağlı, None bırakırsan .env defaults kullanılır) ─────────

CHUNK_SIZE   = 200    # token cinsinden
OVERLAP      = 50     # token cinsinden
SHORT_DOC_MAX = 0     # 0 = her zaman bol (gozlemlemek icin)
...
def inspect(label: str, text: str) -> None:
    ...
    print(f"\n{SEPARATOR}")
    print(f"  {label}  |  Toplam: {total_tokens} token  |  {len(text.strip())} karakter")
    print(SEPARATOR)
    ...
    for i, c in enumerate(chunks):
        ...
        print(f"  Chunk {i+1:02d}  [{c.token_count:>4} token]  {bar}")
        print(f"  {c.text[:200].replace(chr(10), ' | ')}")
        if len(c.text) > 200:
            print(f"  ... (+{len(c.text)-200} karakter daha)")
        print()

    if len(chunks) < 2:
        print("  [Overlap testi icin en az 2 chunk gerekli]\n")
        return

    # --- CUMLE KESME KONTROLU ---
    print("  --- CUMLE KESME KONTROLU ---")
    sentence_end = {".", "!", "?", '."', '!"', '?"', "...", ":"}
    cut_count = 0
    for i, c in enumerate(chunks):
        text = c.text.strip()
        ...
        status_end = "OK " if ending_ok else "KES"
        status_start = "OK " if (i == 0 or starting_ok) else "KES"
        if status_end == "KES" or status_start == "KES":
            cut_count += 1
        print(f"\n  Chunk {i+1:02d}  [bitis:{status_end}] [baslangic:{status_start}]")
        print(f"    Son 60 kar  : ...{text[-60:].replace(chr(10), ' ')}")
        print(f"    Ilk 60 kar  : {text[:60].replace(chr(10), ' ')}...")

    print(f"\n  Sonuc: {cut_count} chunk'ta olasi cumle kesimi var / {len(chunks)} chunk")
    ...
    for i in range(len(chunks) - 1):
        ...
        print(f"\n  Chunk {i+1} sonu  : ...{prev_tail}")
        print(f"  Chunk {i+2} basi  : {next_head}...")
        print(f"  Ortak kelime: {len(shared)}  (~{overlap_token_est} token)")
