"""
Sherlock Holmes RAG test — /api/v1/chat/stream endpoint.

Sends all questions from sherlock_questions.md (baseline, multi-retrieval,
hallucination) and saves full results to scripts/responses/.
...
def fetch_chunks(query: str) -> list[dict]:
    """
    Call rag_search on the tool server directly with the raw question text.
    Returns a list of {"index": int, "preview": str, "full_text": str}.

    Note: the LLM may rephrase its rag_search query differently at runtime.
    This shows what the retrieval system returns for the original question.
    """
    try:
        r = httpx.post(
            TOOL_SERVER_URL,
...
def ask_chat_stream(question: str, conversation_id: str) -> tuple[str, float]:
    """Send one question to /chat/stream and return (full_answer, duration_s)."""
    payload = {
    ...
    return "".join(parts), round(time.monotonic() - t0, 2)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Comma-separated question IDs to run, e.g. Q1,Q2,H1. Runs all if omitted.",
    )
    ...
    out_file = out_dir / f"sherlock_chat_{timestamp}.json"
    ...
    for i, q in enumerate(questions):
        ...
        chunks: list[dict] = []
        try:
            chunks = fetch_chunks(q["text"])
            print(f"  Chunks retrieved: {len(chunks)}")
            for c in chunks:
                print(f"    [{c['index']}] {c['preview']}")

            answer, duration = ask_chat_stream(q["text"], conv_id)
        ...
        # Save after every question so partial results survive a crash.
